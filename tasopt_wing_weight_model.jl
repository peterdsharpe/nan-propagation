...
# out = surfw(
#     po=0.0, # Point where loads and stresses are calculated.
#     b=60.0, # Wingspan.
#     bs=15.0, # Spanwise location of the start of the taper.
#     bo=6.0, # Spanwise location of the root chord.
#     co=6.0, # Root chord length.
#     zs=-5.0, # Height of the strut attach point above wing.
#     lambdat=0.4, # Tip chord ratio (tip chord / root chord).
#     lambdas=1.0, # Start chord ratio (start chord / root chord).
#     gammat=0.0, # Tip airfoil section shape exponent.
#     gammas=0.0, # Start airfoil section shape exponent.
#     Nload=1, # Number of loads (used to distribute engine loads).
#     iwplan=0, # Indicates the presence of a strut.
#     We=0.0, # Weight of the engine.
#     neout=0, # Number of outboard engines.
#     dyeout=5.0, # Distance between engines and the wingtip.
#     neinn=1, # Number of inboard engines.
#     dyeinn=10.0, # Distance between engines and the wing root.
#     Winn=5000. * 9.81, # Weight of inboard engines.
#     Wout=0.0, # Weight of outboard engines.
#     dyWinn=0.0, # Weight distribution of inboard engines.
#     dyWout=0.0, # Weight distribution of outboard engines.
#     sweep=30.0, # Sweep angle in degrees.
#     wbox=2.0, # Width of the wing box.
#     hboxo=0.8, # Height of the wing box at the root.
#     hboxs=0.5, # Height of the wing box at the strut attach point.
#     rh=0.5, # Fractional height of the wing box.
#     fLt=1.0, # Factor applied to the tip load.
#     tauweb=330e6, # Web material shear strength.
#     sigcap=450e6, # Cap material axial compressive strength.
#     sigstrut=450e6, # Strut material axial compressive strength.
#     Ecap=70e9, # Cap material Young's modulus.
#     Eweb=70e9, # Web material Young's modulus.
#     Gcap=25e9, # Cap material shear modulus.
#     Gweb=25e9, # Web material shear modulus.
#     rhoweb=2700.0, # Density of the web material.
#     rhocap=2700.0, # Density of the cap material.
#     rhostrut=2700.0, # Density of the strut material.
#     rhofuel=800.0 # Density of the fuel.
# )
...
po, b, bs, bo, co, zs,
            ...
            rhoweb, rhocap, rhostrut, rhofuel = [114115.09920909579, 37.53365209643924, 10.697090847485182, 3.6068, 6.232215465340908, 3.9116, 0.25, 0.7, 0.225, 0.8665999999999999, 3.0, 1.0, 30112.820951465743, 0.0, 0.0, 0.0, 0.0, 49916.39401487996, 70827.81179095857, 78177.9198258339, 335574.9479578516, 26.0, 0.5, 0.1268, 0.1266, 0.75, -0.05, 1.378951577903156e8, 2.0684273668547338e8, 2.0684273668547338e8, 6.894757889515779e10, 6.894757889515779e10, 2.6518299575060688e10, 2.6518299575060688e10, 2700.0, 2700.0, 2700.0, 817.0]
...
Ss, Ms, tbwebs, tbcaps, EIcs, EIns, GJs,
        ...
        Wscen, Wsinn, Wsout, dxWsinn, dxWsout, dyWsinn, dyWsout,
        Wfcen, Wfinn, Wfout, dxWfinn, dxWfout, dyWfinn, dyWfout,
        Wweb, Wcap, Wstrut, dxWweb, dxWcap, dxWstrut = surfw(po, b, bs, bo, co, zs,
            λt, λs, γt, γs,
            Nlift, iwplan, Weng1,
            nout, yout, nin, yinn,
            Winn, Wout, dyWinn, dyWout,
